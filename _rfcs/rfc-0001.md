...
It's the expectation of the Society that at this point the RFC's Champion either address feedback and submit for approval again, or accept that concensus will not be reached and withdraw the proposal.
...
From time to time, and RFC may out live it's usefulness. When this occurs any member may post on the mailing list of the overseeing Workingr Group explaining why it should be deprecated, and requesting a vote to deprecate the RFC. Should the vote pass, then the RFC is deprecated and it's no longer in affect or implemented by the Society. The RFC should be updated in the RFC repo and a note indicating why it was deprecated added.
...
Currently, it's been suggested that voting take place via each Working Groups mailing list and that two thirds of members participating in the vote must vote in favour. Members would have two weeks to vote, but no quorum threshold has been discussed or established.
